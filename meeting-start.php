...
$file=fopen('/tmp/meeting_'.$_SESSION['cong'].'_admin.call','w');
	...
	exec("arecord -f S16_LE -r ".$sound_quality." | ".$lame_bin." --preset cbr  -b ".$bitrate." -m m -S - - | ".$ezstream_bin." -c ".$web_server_root."kh-live/config/asterisk-ezstream-".$_SESSION['cong'].".xml > /dev/null &");
	exec("arecord -f S16_LE -r ".$sound_quality." | ".$lame_bin." --preset cbr -b ".$bitrate." -m m -S - ".$web_server_root."kh-live/records/".$_SESSION['cong'].'-'.date('Ymd',time()).'_'.date('His',time()).'.mp3'." > /dev/null &");
	}else{
	exec("arecord -f S16_LE -r ".$sound_quality." | ".$lame_bin." --preset cbr -b ".$bitrate." -m m -S - - | ".$ezstream_bin." -c ".$web_server_root."kh-live/config/asterisk-ezstream-".$_SESSION['cong'].".xml > /dev/null &");
	}
