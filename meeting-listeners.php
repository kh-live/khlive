<?PHP
	...
	'8**Listener (over stream - with a duplicated name)**start_cong**stream-start_cong.ogg**'.time().'**normal****8**User1**',
	'9**Listener (over stream - with a duplicated name)**start_cong**stream-start_cong.ogg**'.time().'**normal****9**User2**',
	...
	$db2=file("db/users");
		...
		$data2=explode ("**",$line2);
		if ($data2[0]==$data[1]) $_SESSION[$temp_user]=$data2[2];
		}
	}
	//if for some reason we can't find it, we default to the name given to edcast
	if (!isset($_SESSION[$temp_user])) $_SESSION[$temp_user]=$data[1];
	//if it's a generic account, we add the name provided by listener
	if (@$data[8]!='') $_SESSION[$temp_user]=$data[1].' ('.$data[8].')';
	...
	$output.= '<div class="'.$user_class.'" id="'.$temp_user.'"><img src="./img/phone1.png" /><div class="live_user_name">'.$_SESSION[$temp_user].' - <a class="stop" href="./meeting-ajax.php?kill=1&user='.$data[1].'">x</a></div></div>';
	}elseif($data[3]=="phone_record"){
	//listening to a recording while the meeting is on... shouldnt happen
	$output.= '<div class="'.$user_class.'" id="'.$temp_user.'"><img src="./img/phone_record.png" /><div class="live_user_name">'.$_SESSION[$temp_user].'</div></div>';
	}else{
	//this is streaming
	$output.= '<div class="'.$user_class.'" id="'.$temp_user.'"><h1 class="user_count">'.$data[6].'</h1><img src="./img/comp1.png" /><div class="live_user_name">'.$_SESSION[$temp_user].'</div></div>';
	}
	}elseif(strstr($data[5],"request")){
	if($data[3]=="phone_live"){
	$important_output.= '<div class="'.$user_class.'" id="'.$temp_user.'"><a class="live_user_link" href="./answer.php?ajax_meeting_page=ok&action=answering&client='.urlencode($data[1]).'&cong='.$data[2].'&type='.$data[3].'&conf='.$data[0].'"><img src="./img/phone2.png" /><div class="live_user_name"><b>'.$_SESSION[$temp_user].'</b></div></a></div>';
	}elseif($data[3]=="phone_record"){
	//listening to a recording while the meeting is on... shouldnt happen
	// cant answer
	}else{
	...
	$tmp=explode("--",$data[5]);
	$paragraph=$tmp[1];
	$important_output.= '<div class="'.$user_class.'" id="'.$temp_user.'"><a class="live_user_link" href="./answer.php?ajax_meeting_page=ok&action=sms_a&client='.$data[1].'&cong='.$data[2].'"><img src="./img/comp2.png" /><div class="live_user_name"><b>'.$_SESSION[$temp_user].'</b><br /><i style="color:rgba(0,0,0,0.7)">(Answer to :'.urldecode($paragraph).')</i></div></a></div>';
	}
	}elseif(strstr($data[5],"answering")){
	if($data[3]=="phone_live"){
	$most_important_output.= '<div class="'.$user_class.'" id="'.$temp_user.'"><a class="live_user_link" href="./answer.php?ajax_meeting_page=ok&action=stop&client='.urlencode($data[1]).'&cong='.$data[2].'&type='.$data[3].'&conf='.$data[0].'"><img src="./img/phone3.png" /><div class="live_user_name"><b>'.$_SESSION[$temp_user].'</b></div></a></div>';
	}elseif($data[3]=="phone_record"){
	//listening to a recording while the meeting is on... shouldnt happen
	// cant answer
	}else{
